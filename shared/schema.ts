...
export const documents = pgTable("documents", {
  ...
  versionType: text("version_type").notNull(), // system_generated, further_produced, fee_earner_amended, fee_earner_approved
  createdAt: timestamp("created_at").notNull().defaultNow(),
  createdBy: varchar("created_by").notNull().references(() => users.id),
...
export const meetingImports = pgTable("meeting_imports", {
  ...
  consentElapsedSeconds: integer("consent_elapsed_seconds"), // Seconds into recording when verbal consent was confirmed
...
export const insertAudioRecordingSchema = createInsertSchema(audioRecordings).omit({
  id: true,
  recordedAt: true,
  expiresAt: true, // Server calculates this (7 days from creation)
  deletedAt: true,
}).extend({
  caseId: z.string().uuid(),
...
export const insertFirmProfileSchema = createInsertSchema(firmProfile).omit({
  ...
  phone: z.string().max(50).transform(sanitizeString).optional(),
  email: z.string().email().max(255).transform(sanitizeString).optional().or(z.literal('')),
