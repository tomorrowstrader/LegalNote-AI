...
THE GOVERNING TEST: content is ESTABLISHED if it was said at the meeting, or if it follows from what was said by arithmetic (other than relationship durations), or by applying the correct legal term of art to it, or if it states a system-supplied relationship duration fact that was actually provided. Relationship durations (marriage, cohabitation, total span) are computed by the system when supplied; free-form model date-arithmetic for those durations is not the preferred or expected path. Silence on a cohabitation duration or total relationship span, where the system did not supply one, is correct practice and is not a defect. A statement is defective ONLY if it introduces content that was neither said nor follows from what was said (and is not a system-supplied duration fact). A statement is NOT defective because its exact words were not spoken, and NOT defective because a value it states was computed rather than uttered. A professional legal document re-expresses what was said, in legal register, in standard notation, and with the values that follow from the facts. That is the job. It is not fabrication.
...
states a cohabitation duration or total relationship span in years (or announces that such a duration "could not be established") when the system-supplied duration facts did not provide that figure — inventing a cohabitation or total-span number is a defect; silence on those durations is not;
...
System-supplied relationship duration facts: when the system supplies a marriage, cohabitation or total-span duration figure, that figure is authoritative. A note that states a supplied figure is not a defect — even if a different number could be read from the raw dates. Where the system did not supply a cohabitation duration or total span, remaining silent on those durations is correct and must not be flagged. Do not prefer free-form model date-arithmetic over a system-supplied duration fact.
